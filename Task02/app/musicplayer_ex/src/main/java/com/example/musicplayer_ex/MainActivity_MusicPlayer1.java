...
public class MainActivity_MusicPlayer1 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mediaPlayer = MediaPlayer.create(MainActivity_MusicPlayer1.this, R.raw.heart_thump_pop); //음악파일
    ...
    protected void onResume() { //앱이 다시 실행이 될 때 무조건 실행해야 하는 것은 여기서 실행
        ...
        AudioManager mAudioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE); //오디오매니저 선언
        SharedPreferences pref = getSharedPreferences("pref", Activity.MODE_PRIVATE);

        if (isPaused) { //onPause()에서 저장했던 볼륨값 원상복구!
            mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC,
                    (int) (pref.getInt("vol", vol)),
                    AudioManager.FLAG_PLAY_SOUND);
        } else { //처음엔 볼륨 2로! 미디어 볼륨이 너무 크게 설정된 상태로 들어가면 놀란다
            mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC,
                    (int) (2),
                    AudioManager.FLAG_PLAY_SOUND);
        }
    ...
    protected void onPause() {
        ...
        vol = (int) mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC); //현재 볼륨 저장
        editor.putInt("vol", vol);
        editor.apply();

        //여기서 전화가 왔을 때 MediaVolume 음량을 현재의 50퍼센트로 서서히 줄이자
        //AudioManager 이용
        //Toast.makeText(this, "onPause() 호출됨", Toast.LENGTH_LONG).show();

        mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC,
                (int) (mAudioManager.getStreamVolume(AudioManager.STREAM_MUSIC) * 0.50),
                AudioManager.FLAG_PLAY_SOUND);
